...
function bandsFromLegacyCurve(curve: unknown[]) {
	return LEGACY_CURVE_BAND_INDEXES.map((index) =>
		normalizeBandValue(curve[index]),
	);
}

export function normalizeGroundEqSettings(
	value:
		| (Partial<StoredGroundEqSettings> & { curve?: unknown[] })
		| null
		| undefined,
): StoredGroundEqSettings {
	const source = Array.isArray(value?.bands)
		? value.bands
		: Array.isArray(value?.curve)
			? bandsFromLegacyCurve(value.curve)
			: defaultGroundEqBands;
	const bands = Array.from({ length: GROUND_EQ_BAND_COUNT }, (_, index) =>
		normalizeBandValue(source[index]),
	);
	return {
		bands,
		...
		floatingBlocksEnabled: normalizeFloatingBlocksEnabled(
			value?.floatingBlocksEnabled,
		),
		floatingBlockIntensity: normalizeFloatingBlockIntensity(
			value?.floatingBlockIntensity,
		),
		floatingBlockMinSize: normalizeFloatingBlockMinSize(
			value?.floatingBlockMinSize,
		),
		floatingBlockMaxSize: normalizeFloatingBlockMaxSize(
			value?.floatingBlockMaxSize,
		),
...
export function writeGroundEqSettingsStorage(settings: StoredGroundEqSettings) {
	...
	window.localStorage.setItem(
		...
		JSON.stringify(normalizeGroundEqSettings(settings)),
	);
}
